...
class ViewRouter{
  ...
  createComponents(){
    ...
    Vue.component("router-view",{
      render:h=>{
        // 容错：hash 指向未注册的路由时（例如手输 #/xxx），
        // 旧实现直接访问 this.routeMap[curr].component 会抛 TypeError → 整页白屏。
        // 这里回退到 "/" 或 "/index"，仍无匹配则渲染可见提示。
        var route = this.routeMap[this.app.curr] || this.routeMap["/"] || this.routeMap["/index"];
        if(!route){
          return h("div",{class:"hm-panel hm-empty"},[
            h("h3","页面不存在"),
            h("p","路由 " + this.app.curr + " 未在后台注册，请从左侧菜单选择功能。")
          ]);
        }
